...
var globalHostBindingsSchema = exports_external.object({
  ...
  randomSeed: exports_external.function().args(exports_external.union([exports_external.literal(Mode.DON), exports_external.literal(Mode.NODE)])).returns(exports_external.number()),
...
function callCapability({ capabilityId, method, mode = Mode.DON, payload }) {
  if (mode === Mode.DON)
    runtimeGuards.assertDonSafe();
  if (mode === Mode.NODE)
    runtimeGuards.assertNodeSafe();
...
function runInNodeMode(fn, consesusAggretation, unwrapOptions) {
  return async (...args) => {
    const nodeRuntime2 = runtime.switchModes(Mode.NODE);
...
var configHandler = async ({ configParser, configSchema } = {}) => {
...
var configSchema = exports_external.object({
  ...
  evms: exports_external.array(exports_external.object({
    storageAddress: exports_external.string(),
    calculatorConsumerAddress: exports_external.string(),
    chainSelector: exports_external.string(),
    gasLimit: exports_external.string()
  }))
});
async function fetchMathResult(_, config) {
  const response = await cre.utils.fetch({
    url: config.apiUrl
  });
  return Number.parseFloat(response.body.trim());
}
var onCronTrigger = async (config, runtime4) => {
  ...
  runtime4.logger.log("Successfully fetched offchain value");
  const evmConfig = config.evms[0];
  const evmClient = new cre.capabilities.EVMClient(undefined, BigInt(evmConfig.chainSelector));
  ...
  runtime4.logger.log(`Successfully read onchain value: ${onchainValue.toString()}`);
  const offchainBigInt = BigInt(Math.floor(offchainValue));
  const finalResult = onchainValue + offchainBigInt;
  runtime4.logger.log("Final calculated result");
  runtime4.logger.log("Updating calculator result...");
  ...
  runtime4.logger.log("Dry running call to ensure the value is not anomalous...");
  ...
  runtime4.logger.log(`Dry run response: ${dryRunResponse ? "Anomalous" : "Not anomalous"}`);
  if (dryRunResponse) {
